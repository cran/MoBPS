  ...
  if(length(database)>0){
    if(length(time.point)==1){
      for(index in 1:nrow(database)){
        population$breeding[[database[index,1]]][[database[index,2]+10]][database[index,3]:database[index,4]] <- time.point
      }
    } else{
      next1 = 1
      for(index in 1:nrow(database)){
        n_indi = database[index,4] - database[index,3] + 1
        population$breeding[[database[index,1]]][[database[index,2]+10]][database[index,3]:database[index,4]] <- time.point[next1:(next1 + n_indi - 1)]
        next1 = next1 + n_indi
      }

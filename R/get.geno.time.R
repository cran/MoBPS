...
#' @return genotyping timepoint
#' @export

get.geno.time <- function(population, database=NULL, gen=NULL, cohorts=NULL, id = NULL, use.id=TRUE, use.all.copy = TRUE){
  ...
  if(use.all.copy){
        ...
        for(index in database[row,3]:database[row,4]){
          ...
          for(rows in 1:nrow(copies)){
            if(population$breeding[[copies[rows,1]]][[copies[rows,2]]][[copies[rows,3]]][[16]]==1){
              data[rindex] <- min(data[rindex], population$breeding[[copies[rows,1]]][[44+ copies[rows,2]]][copies[rows,3]], na.rm = TRUE)
            }
          }
          rindex <- rindex + 1
        }
    ...
    for(row in 1:nrow(database)){
      ...
      if(nanimals>0){
        data[(before+1):(before+nanimals)] <- population$breeding[[animals[1]]][[44+ animals[2]]][animals[3]:animals[4]]

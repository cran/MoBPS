...
#' @return Avg. phenotype of the offspring of in gen/database/cohorts selected individuals
#' @export

get.pheno.off <- function(population, database=NULL, gen=NULL, cohorts=NULL, id = NULL, use.id=TRUE){
  ...
  for(row in 1:nrow(database)){
    ...
    if(nanimals>0){
      data[,(before+1):(before+nanimals)] <- population$breeding[[animals[1]]][[26+ animals[2]]][, animals[3]:animals[4]]

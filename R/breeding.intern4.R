...
#' @export
#'
breeding.intern4 <- function(info.parent, parent,  population , mutation.rate = 10^-5, remutation.rate = 10^-5, recombination.rate=1,
  ...
  tmp1 = max(new.poc[length(new.poc)],-2)
  tmp2 = logical(0)
  ...
  for(index in 1:(length(porc)-1)){
    ...
    if(length(activ.origin)>0){
      while(tmp3>length(parent[[activ+4]])){ # add -1 avoid max(numeric(0))
        activ.origin <- activ.origin[-length(activ.origin)]
        tmp3 = max(activ.origin,-1)
      }
    }

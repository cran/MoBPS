...
get.pool.founder <- function(population, database=NULL, gen=NULL, cohorts=NULL, id = NULL, use.id=TRUE){
  ...
  for(row in 1:nrow(database)){
    ...
    if(nanimals>0){
      data[(before+1):(before+nanimals)] <- population$breeding[[animals[1]]][[36+ animals[2]]][animals[3]:animals[4]]
  ...
  if(sum(is.na(data))>0){
    unassigned = 0
    per_snp_pool = get.pool(population, database = database)
    for(index in which(is.na(data))){
      activ = as.numeric(per_snp_pool[,c(index*2-1, index*2)])
      if(prod(activ==activ[1])==1){
        data[index] = activ[1]
      } else{
    ...
    if(unassigned>0){
      warning(paste0(unassigned, " individuals have contributions from multiple pools! No founder pool return for those individuals"))
    }
  }

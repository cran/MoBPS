...
get.death.point <- function(population, database=NULL, gen=NULL, cohorts=NULL, id = NULL, use.id=TRUE){
  ...
  for(row in 1:nrow(database)){
    ...
    if(nanimals>0){
      data[(before+1):(before+nanimals)] <- population$breeding[[animals[1]]][[24+ animals[2]]][animals[3]:animals[4]]

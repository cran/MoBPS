...
#' @export
#'
breeding.intern6 <- function(info.parent, parent,  population , mutation.rate = 10^-5, remutation.rate = 10^-5, recombination.rate=1,
  ...
  for(index in 1:(length(porc)-1)){
    ...
    start <- start_list[[activ]][index]
    tmp3 = (start+sum(activ.porc))
